/**
 * Author: nor
 * Source: Harold N. Gabow, "The Weighted Matching Approach to Maximum Cardinality Matching" (2017) (https://arxiv.org/abs/1703.03998)
 * Description: Matching for general graphs. 1-indexed vertices.
 * Time: O(\sqrt{n} m \log_{\max\{2, 1 + m/n\}} n)
 * Memory: O(n + m)
 * Status: tested at Graph of Love, 900ms for functional graph of 10^5 vertices
 */
#pragma once
...
class MaximumMatching {
   ...
   public:
    ...
    vector<Edge> get_edges() {
        vector<Edge> ans;
        for (int i = 1; i <= N; ++i) if (mate[i] > i) ans.push_back(Edge{i, mate[i]});
        return ans;
    }
   private:
    void reset_count() {
